...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (authUsersError) {
      ...
      return new Response(JSON.stringify({ error: `Failed to list users: ${authUsersError.message}` }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500,
      });
    }
    ...
    const authUsersMap = new Map(authUsersData.users.map(user => [user.id, user.email]));

    const usersWithProfiles = (profilesData || []).map(profile => ({
      id: profile.id,
      email: authUsersMap.get(profile.id) || "Email tidak ditemukan",
    ...
    console.error("Unexpected error in list-users-by-admin:", error.message || error);
