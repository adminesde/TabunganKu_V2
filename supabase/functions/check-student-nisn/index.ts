...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (error && error.code !== "PGRST116") { // PGRST116 means no rows found
      console.error("Error querying student by NISN:", error.message);
    ...
    const studentName = data ? data.name : null;

    return new Response(JSON.stringify({ studentName }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });

  } catch (error: any) {
    console.error("Unexpected error in check-student-nisn:", error.message || error);
