...
export default function ParentViewClassSchedulePage() {
  ...
  useEffect(() => {
    ...
    const fetchStudentsForSchedule = async () => {
      ...
      // First, get student IDs linked to this parent
      ...
      if (parentStudentsError) {
        setError(parentStudentsError.message);
        showErrorToast("Gagal memuat data anak: " + parentStudentsError.message);
        setLoading(false);
        return;
      }
      ...
      let query = supabase
        ...
        .in("student_id", parentStudentIds); // Filter by children's IDs
  ...
  }, [session, selectedClass, amountExpected, frequency, dayOfWeek]); // Removed supabase from dependencies
  ...
  if (error) {
    return <StatusCard status="error" message={error} backButtonHref="/parent/saving-schedules" backButtonText="Kembali ke Jadwal Menabung" />;
  }

  return (
    <PageCardLayout
      title="Detail Jadwal Menabung Anak"
      backHref="/parent/saving-schedules"
      className="max-w-4xl"
    >
      <div className="space-y-4 mb-6">
        <p><strong>Kelas:</strong> {selectedClass}</p>
        <p><strong>Jumlah Diharapkan:</strong> Rp {parseFloat(amountExpected || '0').toLocaleString('id-ID')}</p>
        <p><strong>Frekuensi:</strong> {frequencyMap[frequency || ''] || '-'}</p>
        {dayOfWeek && <p><strong>Hari:</strong> {dayOfWeekMap[dayOfWeek] || dayOfWeek}</p>}
      </div>

      <h3 className="text-xl font-semibold mb-4">Anak dalam Jadwal Ini ({students.length})</h3>
      {students.length === 0 ? (
        <p className="text-center text-muted-foreground">Tidak ada anak Anda yang ditemukan dengan jadwal ini di kelas {selectedClass}.</p>
      ) : (
